	...
	for(i = 0; i < length; i ++){
		...
		//		}
			...
			if(degen == 1){
				...
				for(n = 0; n < 6; n++){
					qn[i * 6 + n] = Qin[n] + dt*(L1 * Qelas[n] + L2 * Qelas2[n] + L3 * Qelas3[n] + L4 * Qelas4[n] + chiral * 2 * (L1 - S * third * L3) * qch * Qch[n] - 2 * Wstr * Qdiff[n]);
				}
			}
			else if(degen == 0 && inf == 0){
				for(n = 0; n < 6; n++){
					qn[i * 6 + n] = Qin[n] + dt*(L1 * Qelas[n] + L2 * Qelas2[n] + L3 * Qelas3[n] + L4 * Qelas4[n] + chiral * 2 * (L1 - S * third * L3) * qch * Qch[n] - Wstr* (Qin[n]-qo_p[nb * 6 + n]));
				}
			}
			nb ++;
		}
