...
bool conf(double **pos){
	/* Initialize the configuration based on different seed input*/
        int l;
        int nb, nd;
        int i, j, k, n;
	...
	//escaped cylinder
        ...
        //DSS or RSS initial configuration
        else if(seed == 32 || seed == 33){
                l = 0;
		nd = 0;
	        double qch_ini = 0.5 * rand_seed / R * M_PI;
                for(k = 0; k < Nz; k++){
                        for (j = 0; j < Ny; j++){
                                for (i = 0; i < Nx; i++){
                                        if(drop[l] || boundary[l]||nboundary[l]){
                                                ...
                                                z = (k-rz)*dz;
                                                dis = sqrt(x*x+y*y+z*z);
                                                if(seed == 32)   omega = dis * qch_ini;
                                                else
                                                        omega = atan2(y, x) + dis * qch_ini;
                                                disxy = sqrt(x * x + y * y);
                                                if(disxy == 0){
                                                        dir[2] = 1;
                                                        dir[0] = dir[1] = 0;
