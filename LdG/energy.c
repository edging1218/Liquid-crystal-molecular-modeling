...
double energy_ldg(){
	/*Landau de-Gennes phase energy for bulk points.*/
	int i, n;
	double ans = 0;
	double traceqq = 0;
	double Qin[6] = {0};
...
void energy_el(double* ans){
	...
	ans[2] *= 0.5 * dV * L3;
	ans[3] *= 0.5 * dV * L4;
	ans[4] *= dV * chiral * 2 * (L1 - S * L3 / 3.0) * qch;
}

void energy_surf(double* ans){
	/*Surface energy for bulk points*/
	int i, n, nb;
	double Qdiff[6] = {0};
	double Qin[6] = {0};
	double loc_nu[3] = {0};
...
void en_degen(double* Qin, double* loc_nu, double* Qdiff){
	/*Helper function for the calculation of surface energy for degenerate planar anchoring*/
	int i, n, j, l, m;
	double Qtemp[3][3];
	double ptemp[3][3];
	double Qp[3][3];
