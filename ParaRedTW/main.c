...
int main(int argc, char *argv[]){
	...
	int Nx2, Ny2, Nz2;
	int x, y, z;
	char showcolor, showssb, showstw;
	float vcolor[3] = {0};
	float nu[3] = {0};
				...
				for (i = 0; i < Nx; i++){
					...
					if(indx[l] > -2){
						if(indx[l] == 0){
							...
							for (n = 0; n < 6; n++) {
								Qin[n] = q[l * 6 + n];
								dQ[0][n] = (q[xp * 6 + n] - q[xm * 6 + n]) * 0.5;
								dQ[1][n] = (q[yp * 6 + n] - q[ym * 6 + n]) * 0.5;
								dQ[2][n] = (q[zp * 6 + n] - q[zm * 6 + n]) * 0.5;
							}
							stw =  Qin[0] * (dQ[1][2] - dQ[2][1]) + Qin[1] * (dQ[2][0] - dQ[0][2]) + Qin[2] * (dQ[0][1] - dQ[1][0])\
							    +  Qin[1] * (dQ[1][4] - dQ[2][3]) + Qin[3] * (dQ[2][1] - dQ[0][4]) + Qin[4] * (dQ[0][3] - dQ[1][1])\
							    +  Qin[2] * (dQ[1][5] - dQ[2][4]) + Qin[4] * (dQ[2][2] - dQ[0][5]) + Qin[5] * (dQ[0][4] - dQ[1][2]);
						}
						fprintf(file, "\t%f\n", stw);
					}
				}
